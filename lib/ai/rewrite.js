...
const MAX_CHAR_DELTA = Number.parseInt(ENV('AI_REWRITE_MAX_CHAR_DELTA', '240'), 10)
const MAX_LENGTH_RATIO = Number.parseFloat(ENV('AI_REWRITE_MAX_LENGTH_RATIO', '2.4'))
const MIN_LENGTH_RATIO = Number.parseFloat(ENV('AI_REWRITE_MIN_LENGTH_RATIO', '0.3'))
...
const buildMessages = ({ projectTitle, language, segments }) => {
  const system = {
    role: 'system',
    content: `You clean up noisy subtitle drafts.
- Rewrite each line so it reads naturally and professionally, keeping speaker intent.
- Remove obvious garbage words, repeated letters, and incorrect spellings.
- Fix grammar, punctuation, and capitalization.
- Preserve proper nouns if you can infer them. If unsure keep the safer word.
- Keep each line concise (generally < 20 words).
- Return only JSON matching: {"segments": [{"index": number, "rewrite": string, "confidence": number, "notes": string|null}]}
- Skip segments when no rewrite is needed.
